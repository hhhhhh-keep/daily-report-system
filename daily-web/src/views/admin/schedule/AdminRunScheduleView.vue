...
async function savePeriod(period: AnalysisPeriodConfiguration) { period.emailRecipients = (recipientTexts.value[period.period] || '').split(',').map(value => value.trim()).filter(Boolean); period.emailCcRecipients = (ccRecipientTexts.value[period.period] || '').split(',').map(value => value.trim()).filter(Boolean); try { const updated = (await adminApi.updateAnalysisPeriodConfiguration(period)).data; periods.value = periods.value.map(value => value.period === updated.period ? updated : value); saved(`${names[updated.period]}计划已保存`) } catch (error) { failed(error) } }
onMounted(load)
</script>
